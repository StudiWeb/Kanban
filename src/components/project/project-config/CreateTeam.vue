<template>
    ...
    <div class="row">
        <div class="col-xl-6 pr-0">
            ...
            <div class="px-0 form-group">
                ...
                <small v-if="componentName === 'EditProject'" class="form-text text-muted">If you want to change team name, please go to 'Manage team' and then 'Edit team'.</small>
            ...
            <div class="d-flex flex-column flex-lg-row align-items-md-start">
                ...
                <the-employees :title="teamName">
                    <template #managers>
                        <div class="my-2">
                            <div>
                                <span>Project manager: </span><span>{{projectManagerName}}</span>
                            </div>
                            <div>
    ...
    export default {
        ...
        props: ['componentName','projectManager','projectMembers'],
    
        emits: ['open-modal','change-key','project-form','update-team'],
        ...
        mounted() {
            ...
            //gets all employees and team leaders
            ...
            //inits form data for Edit project
            get(child(ref(database), 'projects/' + this.projectId)).then((snapshot) => {
            if (snapshot.exists()) {
                const project = snapshot.val();
                const name = snapshot.val().name;
                const startDate = snapshot.val().startDate;
                const endDate = snapshot.val().endDate;
                const projectManagerId = snapshot.val().projectManager.id;
                const teamName = snapshot.val().team.name;
                const teamMembers = snapshot.val().team.members;
                const teamLeaderId = snapshot.val().team.members.find((m) => m.isSelectedAsTeamLeader).id;
                this.enteredTeamName = teamName;
                this.selectedTeamLeaderId = teamLeaderId;
                this.selectedTeamLeader = snapshot.val().team.members.find((m) => m.id === teamLeaderId);
                ...
                //removes team members from employees
                this.teamMembers.forEach((t) => {
                    while(this.employees.find((e) => e.id === t.id)) {
                        const index = this.employees.findIndex((e) => e.id === t.id);
                        this.employees.splice(index,1);
                ...
                this.$emit('project-form',
                    name, startDate, endDate, projectManagerId,teamMembers,project,teamName
                );
